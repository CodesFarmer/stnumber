...
//    generator->generate_patches_crop(file_lists, 24, dest_path, GeneratePatch::HDF5);


    assert(argc == 3);
    std::string file_lists(argv[1]);
    std::string dest_path(argv[2]);
    boost::shared_ptr<GeneratePatch> generator(new GeneratePatch(2, 0));
    ...
    std::vector<float> mean_value(2, 0.0);
